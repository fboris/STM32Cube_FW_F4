...
/*  
  ...
  In order to avoid data-loss, a 128 bytes buffer is used (InternalBuffer): 
...
AUDIO_ErrorTypeDef AUDIO_REC_Process(void)
{
  ...
  case AUDIO_STATE_PLAY:    
    /* WavFile.fsize = BufferCtl.fptr */
    if(BufferCtl.fptr >= DEFAULT_TIME_REC * DEFAULT_AUDIO_IN_FREQ * DEFAULT_AUDIO_IN_CHANNEL_NBR * 2)
    {
      /* Close file */
      f_close(&WavFile); 
      /* Create a new file system */
      if(f_open(&WavFile, REC_WAVE_NAME, FA_WRITE) != FR_OK)
      {
        while(1);
      } 
      /* Parse the wav file header and extract required information */
      if(WavProcess_HeaderUpdate(pHeaderBuff, &WaveFormat))
      {
      }
      if(f_write(&WavFile, pHeaderBuff, 44, (void*)&byteswritten) != FR_OK)
      {
